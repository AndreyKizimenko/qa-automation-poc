...
| `gitops-free-reset.yml` / `gitops-premium-reset.yml` | Manual, `workflow_call` | Apply the empty `-reset` variant — wipes policies/reports/scripts/profiles on no-team (and Workstations on premium) so the Playwright run starts from a known-blank baseline. |
...
| `nightly-qa-gitops-free.yml` | 05:00 UTC daily, manual | Free chain: apply baseline → verify → apply min → verify → apply reset. |
...
| `playwright-free.yml` / `playwright-premium.yml` | 05:30 UTC daily, manual | Runs the Playwright suite against the matching instance (free runs `@free`-tagged specs only). |
...
| Secret | Used by |
|---|---|
| `FLEET_FREE_URL`, `FLEET_FREE_API_TOKEN` | gitops-free, gitops-free-reset, gitops-verify, playwright-free |
| `FLEET_FREE_ADMIN_EMAIL`, `FLEET_FREE_ADMIN_PASSWORD` | playwright-free |
| `FLEET_FREE_ENROLL_SECRET` | gitops-free, gitops-free-reset (enroll secret managed via gitops on free) |
| `FLEET_PREMIUM_URL`, `FLEET_PREMIUM_API_TOKEN` | gitops-premium, gitops-premium-reset, gitops-verify, playwright-premium |
| `FLEET_PREMIUM_ADMIN_EMAIL`, `FLEET_PREMIUM_ADMIN_PASSWORD` | playwright-premium |
| `FLEET_FREE_SSO_METADATA_URL` | gitops-free, gitops-free-reset |
| `FLEET_PREMIUM_SSO_METADATA_URL` | gitops-premium, gitops-premium-reset |
| `FLEET_EUA_METADATA_URL`, `FLEET_ABM_ORG_NAME`, `FLEET_VPP_LOCATION` | gitops-premium, gitops-premium-reset |
