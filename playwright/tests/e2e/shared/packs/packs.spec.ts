/**
 * Packs CRUD lifecycle (UI-only). Runs against no-team scope so the same
 * flow exercises both free and premium. End-to-end in a single test so
 * a mid-flow failure can't leak state into siblings.
 */
import { test, expect } from '@fixtures';
import { authHeaders, sessionAuthHeaders, assertActivity, apiUrl } from '@helpers/api';

test.describe('Packs', { tag: '@free' }, () => {
