...
// Free has no teams; pass team_id=0 to scope to "no team" globally.
const SCRIPTS_ENDPOINT = '/api/latest/fleet/scripts?per_page=200';

test.describe(`API verify · scripts · ${gitopsLabel}`, () => {
  test('script count matches gitops', async ({ request }) => {
    const res = await request.get(SCRIPTS_ENDPOINT);
  ...
  test('every gitops script exists by basename', async ({ request }) => {
    ...
    for (const script of gitopsConfig.scripts) {
      expect(apiNames, `script "${script.name}" missing from API`).toContain(script.name);
    }
  });
});
