...
| 24 | view-query-details-through-host-details-page | Premium | Report card Actions → Show details → report results page; "View data for all hosts" → full host list | **DONE** | `tests/e2e/premium/hosts/host-report-details.spec.ts` | Split into its own spec after all (the reports-tab spec was already carrying render/search/sort). Needs a **stored result**, so it only works against a real VM; precondition is a durable interval report on the VMs fleet — see BATCH-4.md. Perf-impact cross-check dropped (open question 2). |
...
- **Open questions for the human:**
  ...
  2. ~~`view-query-details` (#24) opens a 2nd tab to cross-check the `/reports` perf-impact column and asserts a "Minimal" tooltip — keep that cross-check, or simplify?~~ **RESOLVED (2026-07-28): simplified.** Shipped as `tests/e2e/premium/hosts/host-report-details.spec.ts` covering the drill itself — card Actions → Show details → this host's stored results (asserting the row the query selected) → "View data for all hosts" → the report's own results page. The perf-impact cross-check was **dropped on purpose**: "Performance impact" is a column on the *reports list* (`QueriesTableConfig.tsx`), not on either page in this flow, so QA Wolf needed a second browser tab to reach it. That makes it a reports-list assertion wearing a host-details costume — it belongs to a reports-list spec if we want it, not to this drill.
